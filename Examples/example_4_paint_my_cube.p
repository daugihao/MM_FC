/*
example_4_paint.mycube.p

This example shows simple application for painting on the cube with several commands. Serves
perfect as script example, which can be placed into MYCUBE AREA using CUBE MANAGER.
Tap from side changes color of brush, tap from top draws with selected brush.
Double tap from top fills the top side. Double tap from side runs small animation
with music. Double tap from bottom erase whole cube and single tap from bottom moves one action back. 

This example also shows more advance work with Push Pop Arrays and Variables. Push Pop Array
is used for storing actions and variable is used for keeping the art-work in non-volatile memory.
If named variable exists, it is loaded every time with animation when the script starts.
...
new icon[]=[ICON_MAGIC1,ICON_MAGIC2,          //MANDATORY START
                                   ...
                                   0,I1,0,    //icon
                                   /******/
                                   ''paint_my_cube'',  //app name sound (does not need to exists)
                                   ''a_memory_exp'']   //explanation name sound (does not need to exists)
